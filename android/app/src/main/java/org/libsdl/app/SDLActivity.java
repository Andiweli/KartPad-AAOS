...
*/
    ...
    // AST AAOS adaptation, 2026-09-07. Parked games must pause when covered,
    // even on Android versions that otherwise allow background multi-window.
    public static boolean isAutomotive() {
        return mSingleton != null && mSingleton.getPackageManager()
                .hasSystemFeature("android.hardware.type.automotive");
    }

    private static boolean supportsBackgroundMultiWindow() {
        return mHasMultiWindow && !isAutomotive();
    }
    ...
    protected void onPause() {
        ...
        if (!supportsBackgroundMultiWindow()) {
            pauseNativeThread();
        }
    }
    ...
    public static void handleNativeState() {
        ...
        if (mNextNativeState == NativeState.RESUMED) {
            if (mSurface.mIsSurfaceReady && (mHasFocus || supportsBackgroundMultiWindow()) && mIsResumedCalled) {
                if (mSDLThread == null) {
                ...
                if (context instanceof Activity) {
                    ...
                    if (window != null) {
                        ...
                        if (Build.VERSION.SDK_INT >= 30 /* Android 11 (R) */) {
                            ...
                            attributes.layoutInDisplayCutoutMode = isAutomotive()
                                    ? WindowManager.LayoutParams.LAYOUT_IN_DISPLAY_CUTOUT_MODE_NEVER
                                    : WindowManager.LayoutParams.LAYOUT_IN_DISPLAY_CUTOUT_MODE_ALWAYS;
                            window.setAttributes(attributes);
                            window.setDecorFitsSystemWindows(isAutomotive());
    ...
    protected boolean sendCommand(int command, Object data) {
        // Native SDL may request fullscreen again after a resize or resume.
        // Clamp the request BEFORE it is queued, not after hiding vehicle UI.
        if (command == COMMAND_CHANGE_WINDOW_STYLE) {
            // This app is immersive on normal Android and window-fitted on AAOS.
            data = Integer.valueOf(isAutomotive() ? 0 : 1);
        }
